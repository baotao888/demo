...
class Tree {
	...
	*/
	 ...
	 * @return array
	 */
	public function get_tree_data($myid = 0, $label = 'org_name'){
		$items = [];
		$child = $this->get_child($myid);
		if(is_array($child)){
			foreach($child as $id=>$value){
				$item = ['label'=>$value[$label], 'id'=>$value['id'], 'data'=>['description'=>$value['description'], 'value'=>$value]];
				$chrildren = $this->get_tree_data($id);
				if ($chrildren) $item['children'] = $chrildren;
				$this->arr_ret[] = $item;
				$items[] = $item;
			}
		}
	 ...
	 * @return array
	 */
	public function get_sub_children($myid = 0, &$arr_ret){
		$items = [];
		$child = $this->get_child($myid);
		if(is_array($child)){
			foreach($child as $id=>$value){
				$this->get_sub_children($id, $arr_ret);
				$arr_ret[] = $id;
				$items[] = $id;
