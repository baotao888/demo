...
define('UC_USER_MOBILE_EXISTS', -8);
...
class usercontrol extends base {
	...
	function usercontrol() {
		parent::__construct();
		$this->load('user');
		$this->app = isset($this->cache['apps'][UC_APPID])?$this->cache['apps'][UC_APPID]:[];
	}
	...
	function onregister() {
		...
		if(($status = $this->_check_mobile($mobile)) < 0) {
			return $status;
		}
	...
	function onget_user() {
		...
		} elseif ($this->input('ismobile')){
			$status = $_ENV['user']->get_user_by_mobile($username);
		} else {
			$status = $_ENV['user']->get_user_by_username($username);
		}
		if($status) {
			return array($status['uid'],$status['username'],$status['email'],$status['mobile']);
		} else {
			return 0;
		}
	}
	...
	function _check_email($email, $username = '') {
		...
		if($this->settings['emptye'] && $email=='') {
			return 1;
		} elseif(!$_ENV['user']->check_emailformat($email)) {
	...
	function _check_mobile($mobile, $username = '') {
		...
		if($this->settings['emptym'] && $mobile=='') {
			return 1;
		} elseif(!$_ENV['user']->check_mobileformat($mobile)) {
			return UC_USER_MOBILE_FORMAT_ILLEGAL;
		} elseif(!$this->settings['doublem'] && $_ENV['user']->check_mobileexists($mobile, $username)) {
